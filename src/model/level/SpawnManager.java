...
public class SpawnManager
{
	...
	private final Random random;	private final int numberOfPlatforms;
	...
	public SpawnManager(AbstractGameObjectFactory factory)
	{
		...
		this.random = new Random();		this.numberOfPlatforms = 10;
	...
	}	public void generateInitialLevel(GameModel model)
	{
		...
		for (int i = 0; i < numberOfPlatforms; i++)
		{
			...
			currentY -= gap;			float platformWidth = 100;
			float x = random.nextFloat() * (model.getScreenWidth() - platformWidth);			Platform p = factory.createRandomPlatform(x, currentY);
			model.getGameObjects().add(p);			if (random.nextFloat() < coinSpawnChance)
			{
